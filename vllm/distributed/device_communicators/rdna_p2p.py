...
def rdna_ar_env_allows_probe(
    rdna_ar: str | None,
    force_custom: bool,
) -> bool:
    """Whether gfx10x should even try to construct rdna_ar.

    Explicit ``0`` wins. Unset follows dest default-off unless
    ``VLLM_FORCE_CUSTOM_ALL_REDUCE`` is set (serve_gfx1030_full.sh).
    """
    if rdna_ar is None or not str(rdna_ar).strip():
        return bool(force_custom)
    mode = str(rdna_ar).strip().lower()
    if mode in _FALSE:
        return False
    return bool(mode in _TRUE or mode == "auto")


def should_init_rdna_ar(
    rdna_ar: str | None,
    force_custom: bool,
    pix_connected: bool,
) -> bool:
    """Whether to run rdna_ar IPC init after topology is known."""
    if not rdna_ar_env_allows_probe(rdna_ar, force_custom):
        return False
    mode = "" if rdna_ar is None else str(rdna_ar).strip().lower()
    if mode == "auto":
        return bool(pix_connected)
    if mode in _TRUE:
        return True
    # Unset + FORCE: try; self-test still rejects a slow PHB/SYS mesh.
    return bool(force_custom)
