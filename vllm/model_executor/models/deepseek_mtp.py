...
class DeepSeekMTP(nn.Module, DeepseekV2MixtureOfExperts, SupportsPP):
    # SupportsPP + embed_tokens load under PP: leapdragon/vllm-rdna2-recipe
    # 0009 (Aron Hsiao), backporting vllm-project/vllm#46994 (Chris Eastwood).
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        super().__init__()
        ...
        # PP: the MTP draft runs only on the last PP stage (the runner gates
        # drafter construction on get_pp_group().is_last_rank), so it never
        # actually consumes PP intermediate tensors — but SupportsPP requires
        # this factory.
