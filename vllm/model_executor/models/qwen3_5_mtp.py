...
class Qwen3_5MultiTokenPredictor(nn.Module):
    ...
    def forward(
        ...
        if pp_group.is_first_rank or pp_group.is_last_rank:
            ...
            # applies exactly as it does at PP=1. Without the last-rank case
            # the draft would fall through to the intermediate-tensor path
            # and project uninitialized state.
...
class Qwen3_5MTP(LocalArgmaxMixin, nn.Module, SupportsMultiModal, SupportsPP):
    # SupportsPP + last-rank embed/fc: leapdragon/vllm-rdna2-recipe 0009
    # (Aron Hsiao), backporting vllm-project/vllm#46994 (Chris Eastwood).
