...
class EngramConfig:
    ...
    """Store embedding weights in CPU memory through the PLE worker.
    ...
    def verify_model_config(self, model_config: "ModelConfig | None") -> None:
        ...
        if (
            ...
            raise ValueError(
                ...
                "embeddings. Currently only the ROCm Qwen4Exp implementation "
    ...
    def verify_parallel_config(self, parallel_config: "ParallelConfig") -> None:
        ...
        if self.embedding_across_dp:
            raise ValueError(
                "This RDNA branch uses a shared CPU PLE table; ETP is unsupported"
            )
        if (
