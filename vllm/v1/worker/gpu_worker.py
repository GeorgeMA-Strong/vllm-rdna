...
class Worker(WorkerBase):
    ...
    def load_model(self, *, load_dummy_weights: bool = False) -> None:
        ...
        if self._ple_offload_enabled:
            from vllm.v1.worker.gpu.model_runner import GPUModelRunner as PLEModelRunner

            cast(PLEModelRunner, self.model_runner)._setup_ple_offload(
    ...
    def reload_weights(self, *args, **kwargs) -> None:
        if self._ple_offload_enabled:
            raise NotImplementedError("Weight reload is not supported with PLE offload")
