...
@pytest.mark.parametrize("extra_draft_group", [False, True])
def test_annotate_eagle_groups_flags_separately_prefixed_drafter_layers(
    ...
    groups = [
        ...
        KVCacheGroupSpec(
            [
                "model.layers.0.attn",
                "model.layers.2.attn",
                "mtp.mtp.layers.3.self_attn.attn",
            ],
            attn,
        ),
    ]
    if extra_draft_group:
        # Flash-Next's MTP attention and indexer caches occupy separate groups.
        kv_cache_spec["mtp.mtp.layers.3.indexer.cache"] = attn
        groups.append(KVCacheGroupSpec(["mtp.mtp.layers.3.indexer.cache"], attn))
    vllm_config = Mock()
    ...
    assert [g.is_eagle_group for g in groups] == [False, True] + (
        [True] if extra_draft_group else []
    )
